...
\end{center}
\caption{\small{On the left is the standard suffix tree for $t =$ BANANA. On the right, the suffix tree has been modified to contain a wildcard subtree at the root. Note that the big space, fast query solution requires that wildcard subtrees be created between every letter.}}
\label{fig:sample1}
\end{figure*}
