...
However, we encountered substantial difficult after initially implementing this approach and running experiments.
...
When encountering a wildcard in a query pattern we perform a rooted LCP query and when encountering non-wildcard characters we perform an unrooted LCP query.
An unrooted LCP query has additional overhead and reduces to a rooted LCP query.
Therefore, in terms of constant factors, an unrooted LCP query should take strictly slower than a rooted LCP query.
